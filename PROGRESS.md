...
| 10 | Error states, remaining tests | Done — 47 frontend tests, 16 contract tests |
...
one rewrite rule (`vercel.json`). Both are deployed from `main`.

## Since the first review round

1. **Visual rework (`48c5405`)** — near-black canvas with a surface lift scale,
   quiet provenance marks instead of warning-coloured pills, a 52px header with
   a bottom nav on mobile, real risk icons with distinct silhouettes, one
   collapsible status line, and monospace tabular figures for all data.
...
3. **Wallet picker** — EIP-6963 discovery, a modal that always opens, and the
   chain guard on connect.

## Where I am

Last files worked on: `src/services/wallet/eip6963.ts`, `catalog.ts`,
`walletService.ts` and `src/components/WalletModal.tsx` — Connect now opens a
wallet picker built on EIP-6963 discovery, with MetaMask, Bitget Wallet and
Rabby always listed, install links for the ones that are missing, a reserved
WalletConnect slot, and the 421614 chain guard applied after connecting.
Verified in a headless browser across four scenarios: no wallet, two announced
wallets, a wallet on Ethereum mainnet, and legacy `window.ethereum` only.

Previously: the visual pass — `src/styles/tokens.css`,
...
- `npm test` — 47 passing. `npm run test:contracts` — 16 passing.
